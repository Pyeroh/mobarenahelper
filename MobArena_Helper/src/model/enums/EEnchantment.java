...
public enum EEnchantment {
	 ...
	 *
	 * @param name
	 *            le nom recherché, il peut être renvoyé par la méthode {@link EEnchantment#getName()}
	 * @return la constante associée au nom passé en paramètre
	 * @throws IllegalArgumentException
	 *             si la constante recherchée n'existe pas
	...
	public static EEnchantment getByName(String name) {
		...
		if (name != null) {
			...
			if (i < values.length) {
				...
				throw new IllegalArgumentException("No enum constant EEnchantment." + name);
			}
		}
		else {
	...
	public static EEnchantment getById(int id) {
		...
		if (i < values.length) {
			...
			throw new IllegalArgumentException("No enum constant EEnchantment for id " + id);
		}
