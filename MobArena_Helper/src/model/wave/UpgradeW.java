 ...
 * existent déjà (s'ils sont similaires, une épée en diamant pour remplacer une épée en fer, ou un enchantement...) et
 * même de nouvelles permissions.
 *
 * @author Pyeroh
 * @see Wave
 */
public class UpgradeW extends Wave {
	...
	public LinkedHashMap<String, Object> getMap() throws ArenaException {
		...
		if (!upset_map.isEmpty())
			map.put("upgrades", upset_map);
		if (give_all_items)
			map.put("give-all-items", give_all_items);
	...
	public boolean sortUpgrades() {
		...
		for (int i = 0; i < next_upgrades.size(); i++) {
			if (next_upgrades.get(i) != upgrades.get(i))
				changed = true;
		}
	...
	public UpgradeSet findUpgradeSet(Classe classe) {
		...
		while (i < upgrades.size()) {
			if (upgrades.get(i).getClasse() == classe)
				break;
			i++;
		}
		if (i == upgrades.size())
